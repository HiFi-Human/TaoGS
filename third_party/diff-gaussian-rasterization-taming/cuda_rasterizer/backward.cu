...
// each Gaussian.
__device__ void computeColorFromDC(int idx, const float* dc, const bool* clamped, const glm::vec3* dL_dcolor, glm::vec3* dL_ddc)
{
	...
	dL_ddirect_color[0] = SH_C0 * dL_dRGB;
}
	...
	glm::vec4* dL_drot)
{
	...
	// Match the forward-path selection: keep the legacy SH code path whenever
	// SH storage exists, even for degree 0, and only fall back to DC-only when
	// the caller truly did not provide SH coefficients.
	if (shs)
		...
		computeColorFromDC(idx, dc, clamped, (glm::vec3*)dL_dcolor, (glm::vec3*)dL_ddc);
