...
__device__ glm::vec3 computeColorFromDC(int idx, const float* dc, bool* clamped)
{
	glm::vec3* direct_color = ((glm::vec3*)dc) + idx;
	glm::vec3 result = SH_C0 * direct_color[0] + 0.5f;
	...
	bool prefiltered)
{
	...
	{
		// Keep the legacy SH path whenever SH storage is present. For degree 0
		// this still uses only the DC term, but preserves the original gradient
		// flow expected by separate dc/rest training setups.
		glm::vec3 result = shs != nullptr
			? computeColorFromSH(idx, D, M, (glm::vec3*)orig_points, *cam_pos, dc, shs, clamped)
			: computeColorFromDC(idx, dc, clamped);
